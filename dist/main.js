   ...
   socket.on('player', (player) => {
      $('#multiplayer').on('click', () => {
         ...
         $(document).on('keypress', function (e) {
            ...
            switch (e.key) {
               case 'w':
                  board.movePlayer(player === 'host' ? playerA : playerB, 'up')
                  break
               case 's':
                  board.movePlayer(player === 'host' ? playerA : playerB, 'down')
                  break
               case 'a':
                  board.movePlayer(player === 'host' ? playerA : playerB, 'left')
                  break
               case 'd':
                  board.movePlayer(player === 'host' ? playerA : playerB, 'right')
                  break
            }
   ...
   $('#singleplayer').on('click', async () => {
      ...
      $('.playBt').on('click', async () => {
         ...
         $(document).on('keydown', function (e) {
            ...
            switch (e.key) {
               case 'w':
                  board.movePlayer(playerA, 'up')
                  break
               case 's':
                  board.movePlayer(playerA, 'down')
                  break
               case 'a':
                  board.movePlayer(playerA, 'left')
                  break
               case 'd':
                  board.movePlayer(playerA, 'right')
                  break
               case '8':
               case 'ArrowUp':
                  board.movePlayer(playerB, 'up')
                  break
               case '5':
               case 'ArrowDown':
                  board.movePlayer(playerB, 'down')
                  break
               case '4':
               case 'ArrowLeft':
                  board.movePlayer(playerB, 'left')
                  break
               case '6':
               case 'ArrowRight':
                  board.movePlayer(playerB, 'right')
                  break
            }
